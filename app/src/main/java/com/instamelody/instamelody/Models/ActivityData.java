package com.instamelody.instamelody.Models;

import com.instamelody.instamelody.R;

/**
 * Created by Saurabh Singh on 12/21/2016.
 */

public class ActivityData {
    public static Integer[] id_= {0,1,2,3,4,5};
    public static Integer[] userProfileImage = {R.drawable.profile1,R.drawable.profile2,
            R.drawable.profile3,R.drawable.profile4,R.drawable.profile5, R.drawable.cover2};
    public static String[] UserNameArray1 = {"Matt Jacob","Joe Lost","Sonya Yo","Luke Skywalker","Harper Cruise","Merrie Jane"};
    public static String[] Topic = {"World Love","Alter Ago","We are the World","Friends with benifit","busy Day","Rise above hate"};
    public static String[] Time = {"1 hr ago","1 day ago","1 week ago","1 month ago","2 months ago","6 months ago"};
}
